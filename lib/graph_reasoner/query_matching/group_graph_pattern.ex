...
defmodule GraphReasoner.QueryMatching.GroupGraphPattern do
  ...
  defp extract_triples_blocks_from_triples_block(
         %Sym{symbol: :TriplesBlock, submatches: submatches} = triples_block
       ) do
    case submatches do
      [_element] ->
        [triples_block]
      ...
      [element, _word, next_triples_block] ->
        [
          %{triples_block | submatches: [element]}
          | extract_triples_blocks_from_triples_block(next_triples_block)
        ]
    end
  end
  ...
  def wrap_in_graph_pattern_not_triples(group_graph_pattern) do
    %Sym{
      symbol: :GraphPatternNotTriples,
      submatches: [
        %Sym{symbol: :GroupOrUnionGraphPattern, submatches: [group_graph_pattern]}
      ]
    }
  end
end
