defmodule Interpreter.Diff.Store do
    ...
    |> Enum.reduce_while({:fail}, fn array, _ ->
      case Interpreter.Diff.Template.fill_array(array, query_string) do
        ...
        filled_array ->
          ...
          {tree, score} =
            Interpreter.Diff.Store.Storage.template_tree_and_score_for_array(array, symbol)
          ...
          if tree do
            ...
            if score > 0.9 do
              Interpreter.Diff.Store.Manipulator.maybe_push_template_solution_for_array(
                array,
                solution,
                symbol,
                0.02
  ...
  def parse_with_local_store(query_string, rule_name, local_template_store) do
    ...
    |> Enum.reduce_while({:fail}, fn array, _ ->
      case Interpreter.Diff.Template.fill_array(array, query_string) do
        ...
        filled_array ->
          {tree, score} =
            Interpreter.Diff.Store.Storage.template_tree_and_score_for_array_sync(
              array,
              rule_name,
          ...
          if tree do
            ...
            new_store =
              if score > 0.9 do
                Interpreter.Diff.Store.Manipulator.maybe_push_template_solution_for_array_sync(
                  array,
                  solution,
                  rule_name,
                  0.02,
  ...
  def maybe_push_solution_sync(solution, chance, rule_name, template_local_store) do
    if :rand.uniform() <= chance do
      Interpreter.Diff.Store.Manipulator.push_single_solution_sync(
        solution,
        rule_name,
...
defmodule Interpreter.Diff.Store.Storage do
  ...
  def handle_call({:template_tree_and_score_for_array, array, symbol}, _, map) do
    ...
    if template do
      {:reply,
       {Interpreter.Diff.Template.tree(template), Interpreter.Diff.Template.score(template)}, map}
    else
      {:reply, {nil, nil}, map}
    end
  end
...
defmodule Interpreter.Diff.Store.Manipulator do
  ...
  def handle_cast({:push_solution, solution}, _) do
    ...
    new_templates =
      ...
      |> Interpreter.Diff.Template.fold_duplicates(limit: 5)
      |> Interpreter.Diff.Template.sort()
    ...
    case new_templates do
      ...
      [new_template | _] ->
        ...
        new_stored_templates =
          ...
          |> Interpreter.Diff.Template.fold_duplicates(limit: 4)
          |> Enum.take_random(7)
        ...
        GenServer.cast(
          Interpreter.Diff.Store.Storage,
  ...
  def push_solution_for_array_sync(array, solution, rule_name, local_template_store) do
    template =
      Interpreter.Diff.Store.Storage.template_for_array_sync(
        array,
        rule_name,
        local_template_store
      )
  ...
  def push_solution_sync(template, rule_name, solution, local_template_store) do
    ...
    new_templates =
      ...
      # folding in case we fetched new matches
      |> Interpreter.Diff.Template.fold_duplicates(limit: 20)
      |> Enum.map(&Interpreter.Diff.Template.cache_score/1)
      |> Interpreter.Diff.Template.sort()
    ...
    # Set the solution
    Interpreter.Diff.Store.Storage.set_templates_sync(
      new_templates,
      rule_name,
      local_template_store
    )
  ...
  def push_single_solution_sync(solution, rule_name, local_template_store) do
    ...
    new_templates =
      ...
      |> Enum.map(&Interpreter.Diff.Template.make_template(solution, &1))
      |> Enum.reject(&match?({:fail}, &1))
      |> Enum.reject(fn %Interpreter.Diff.Template{array_template: var_arr} ->
        Enum.count(var_arr) == 1
      end)
      # in case we match with multiple other queries
      |> Interpreter.Diff.Template.fold_duplicates(limit: 10)
      |> Interpreter.Diff.Template.sort()

    case new_templates do
      [] ->
        Interpreter.Diff.Store.Storage.push_solution_sync(
          solution,
          rule_name,
          local_template_store
        )
      ...
      [new_template | _] ->
        ...
        new_local_template_store =
          Interpreter.Diff.Store.Storage.set_solutions(
            new_solutions,
            rule_name,
            local_template_store
          )
        ...
        new_stored_templates =
          [
            new_template
            | Interpreter.Diff.Store.Storage.templates_sync(rule_name, new_local_template_store)
          ]
          # folding in case we fetched new matches
          |> Interpreter.Diff.Template.fold_duplicates(limit: 4)
          |> Enum.take_random(7)
