...
defmodule ResourceFormat do
  ...
  defimpl Acl.GraphSpec.Constraint.Protocol do
    def matching_quads( %ResourceFormat{} = resource, quads, extra_quads\\[] ) do
      ...
      ResourceFormat.matching_quads( resource, quads, extra_quads, %{} )
    end
  end
  ...
  defp find_matching_resources( %ResourceFormat{ resource_prefix: prefix }, quads, _extra_quads, _options ) do
    ...
    |> Enum.filter( fn (%Iri{iri: iri}) ->
      ...
      if ( iri_byte_size >= ( prefix_byte_size + first_char_byte_size ) ) do
        <<_::binary-size(first_char_byte_size), first_chars::binary-size(prefix_byte_size), _::binary>> = iri
