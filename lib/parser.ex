defmodule Parser do
  ...
  def split_single_form( string ) do
    ...
    { name, ebnf_tokenizer( {:default} , String.codepoints( clause ) ) }
  end
...
#  matches A followed by B. This operator has higher precedence than alternation; thus A B | C D is identical to (A B) | (C D).
  ...
  def ebnf_tokenizer( prev_mode, [ "#" | rest ] ) when prev_mode in [ { :default }, { :bracket } ] do
    ebnf_tokenizer( { :hex_char, prev_mode, [] }, rest )
  end

  def ebnf_tokenizer( { :hex_char, prev_mode, values }, [ char | rest ] ) when (char >= "0" and char <= "9") or ( char >= "a" and char <= "f" ) do
  ...
  def ebnf_tokenizer( { :hex_char, prev_mode, hex_values }, rest ) do
    ...
    [ { :hex_char, encoded_value } | ebnf_tokenizer( prev_mode, rest ) ]
  end
  ...
  def ebnf_tokenizer( { :bracket }, [ "^" | rest ] ) do
    [ { :negation } | ebnf_tokenizer( { :bracket }, rest ) ]
  end
