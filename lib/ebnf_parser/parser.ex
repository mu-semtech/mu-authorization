defmodule EbnfParser.Parser do
  ...
  # { :range }
  def smart_parse_single_tokens( [ { :range }, end_char_block | rest ], { { :in_bracket }, kind, [ prev | rest_prevs ] } ) do
    smart_parse_single_tokens( rest,  { { :in_bracket }, kind, [ { :range, [ prev, end_char_block ] } | rest_prevs ] } )
  end
  ...
  def smart_parse_parens( [ {:close_paren} | rest ], [ { :paren, content }, { :paren, parent_content } | parents ] ) do
    smart_parse_parens( rest, [ { :paren, [ { :paren_group, Enum.reverse( content ) } | parent_content ] } | parents ] )
  end
      ...
      [_no_pipe] ->
        case split_by( enum, { :minus } ) do
          ...
          [minus_left,minus_right] ->
            ...
            [{ :minus, [ transform_side.( minus_left ),
                         transform_side.( minus_right ) ] }]
          _ -> raise "Too many parts given to minus"
        end
      yes_pipe ->
        [{ :one_of, Enum.map( yes_pipe,
             fn
               [x] ->
  ...
  def smart_parse_infixes( { token, items } ) when token in [ :paren_group, :maybe_many, :one_or_more, :maybe  ] do
