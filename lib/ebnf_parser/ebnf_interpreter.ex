defmodule EbnfInterpreter do
  ...
  def all_options(rule, chars, options \\ %{}) do
    EbnfInterpreter.make_generator(rule, chars, [], options)
    |> generate_all_options
  end
  ...
  """
  def eagerly_match_rule(chars, _syntax, {quote_type, string}, options)
      when quote_type in [:single_quoted_string, :double_quoted_string] do
    chars =
      if Map.get(options, :terminal) do
        chars
      else
        ...
        if leftover == chars do
          ...
          case eagerly_match_rule(leftover, syntax, {:maybe_many, [matcher]}, options) do
            {:ok, new_leftover, new_matched_portion, next_matched_rule_info} ->
              {:ok, new_leftover, matched_portion <> new_matched_portion,
               matched_rule_info ++ next_matched_rule_info}
  ...
  def eagerly_match_rule(chars, syntax, {:one_or_more, [matcher]}, options) do
    case eagerly_match_rule(chars, syntax, matcher, options) do
      {:ok, leftover, matched_portion, matched_rule_info} ->
        {:ok, new_leftover, next_matched_portion, last_matched_rule_info} =
          eagerly_match_rule(leftover, syntax, {:maybe_many, [matcher]}, options)

        {:ok, new_leftover, matched_portion <> next_matched_portion,
         matched_rule_info ++ last_matched_rule_info}
    ...
    case eagerly_match_rule(chars, syntax, first, options) do
      {:ok, first_leftover, first_matched_portion, first_rule_info} ->
        case eagerly_match_rule(first_leftover, syntax, rest, options) do
          {:ok, rest_leftover, rest_matched_portion, rest_rule_info} ->
            {:ok, rest_leftover, first_matched_portion <> rest_matched_portion,
             first_rule_info ++ rest_rule_info}
  ...
  def eagerly_match_rule(chars, syntax, {:one_of, choices}, options) do
    ...
    matched_info =
      Enum.map(choices, fn choice -> eagerly_match_rule(chars, syntax, choice, options) end)
  ...
  def eagerly_match_rule([char | chars], _syntax, {character_type, character}, _)
      when character_type in [:hex_character, :character] do
  ...
  def eagerly_match_rule([], _, {character_type, _}, _)
      when character_type in [:hex_character, :character] do
    {:fail}
  end
  ...
  def eagerly_match_rule(chars, syntax, {:symbol, name}, options) do
    ...
    case eagerly_match_rule(chars, syntax, rule, new_options) do
      {:ok, leftover, matched_portion, matched_rules} ->
        {:ok, leftover, matched_portion, [{name, matched_portion, matched_rules}]}
  ...
  def emit({:list, {generator, following_rules, syntax, options}}) do
    ...
    case emit(generator) do
      {new_first_state, {leftover, matched_portion, matched_rules}} ->
        ...
        emit(
          {:list_loop,
           {{new_first_state, child_generator}, {matched_portion, matched_rules},
  ...
  def emit(
        {:list_loop,
         {{top_generator, bottom_generator}, {top_matched_portion, top_rule_info},
          {following_rules, syntax, options}}}
      ) do
    build_child_generator = fn chars ->
      make_generator(following_rules, chars, syntax, options)
    ...
    case emit(bottom_generator) do
      {new_bottom_state, {leftover, matched_portion, matched_rules}} ->
        ...
        {{:list_loop,
          {{top_generator, new_bottom_state}, {top_matched_portion, top_rule_info},
           {following_rules, syntax, options}}},
      ...
      _ ->
        ...
        case emit(top_generator) do
          {new_top_state, {leftover, top_matched_portion, top_matched_rules}} ->
            ...
            emit(
              {:list_loop,
               {{new_top_state, child_generator}, {top_matched_portion, top_matched_rules},
                {following_rules, syntax, options}}}
            )
  ...
  def emit({:one_or_more, {top_generator}, {item, syntax, options}}) do
    case emit(top_generator) do
      {new_state, {leftover, matched_portion, matched_rules}} ->
        ...
        emit({:one_or_more, {new_state, child_generator}, {item, matched_portion, matched_rules}})
  ...
  def emit({:one_or_more, {top_state, child_state}, {item, matched_portion, matched_rules}}) do
    case emit(child_state) do
      {new_child_state, {new_leftover, new_matched_portion, new_matched_rules}} ->
        {{:one_or_more, {top_state, new_child_state}, {item, matched_portion, matched_rules}},
         {new_leftover, matched_portion <> new_matched_portion,
          matched_rules ++ new_matched_rules}}

      _ ->
        emit({:one_or_more, {top_state}, {item, matched_portion, matched_rules}})
    end
  end
  ...
  def emit({:maybe_many, :go_deeper, [], {item, chars, syntax, options}}) do
    ...
    case emit(generator) do
      {top_state, {top_leftover, matched_portion, matched_rules}} ->
        emit(
          {:maybe_many, :go_deeper, [{top_state, {top_leftover, matched_portion, matched_rules}}],
  ...
  def emit(
    ...
    case emit(generator) do
      ...
      _ ->
        {{:maybe_many, :recycle,
          [{top_state, {top_leftover, matched_portion, matched_rules}} | rest],
          {item, chars, syntax, options}}, {top_leftover, matched_portion, matched_rules}}
    end
  end
  ...
  def emit(
    ...
    case emit(top_state) do
      ...
      _ ->
        {{:maybe_many, :recycle, rest, {item, chars, syntax, options}},
