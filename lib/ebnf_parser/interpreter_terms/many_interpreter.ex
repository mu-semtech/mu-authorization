...
defmodule ManyEmitter do
  defstruct [ :element, :state,
              :child_generator, # generator for the first step.
              { :some_generator, :none }, # generator which emits for
                                          # all the next steps.
  ...
  def walk( %ManyEmitter{ child_result: :none,
                          child_generator: generator } = emitter ) do
  ...
  def walk( %ManyEmitter{ child_result: {:ok, _, %Result{ leftover: leftover }},
                          some_generator: :none,
                          element: element,
                          state: state } = emitter ) do
    emitter
    |> Map.put( :some_generator, EbnfParser.GeneratorProtocol.make_generator(
          %Some{ element: element,
                 state: %{ state | chars: leftover } } ) )
    |> walk
  end

  def walk( %ManyEmitter{ child_result: { :ok, _, child_result },
                          some_generator: some_generator } = emitter ) do
    case emit( some_generator ) do
      { :ok, some_state, some_result } ->
        { :ok,
          %{ emitter | some_generator: some_state },
