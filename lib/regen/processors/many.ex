...
defmodule Many do
  ...
  defp ensure_self_generator( %Many{ element: elt,
                                     self_generator: :none,
                                     state: state } = many ) do
    %{ many |
       self_generator: Regen.Constructor.make( elt, state ) }
  end
  ...
  defp ensure_rest_generator( %Many{ rest_generator: :none,
                                     self_generator: self_gen,
                                     element: elt } = many ) do
  ...
  defp emit_rest_result( %Many{ rest_generator: rest_gen } = many ) do
    case Regen.Protocol.emit( rest_gen ) do
      { :ok, new_rest_gen, result } ->
        { :ok,
          %{ many | rest_generator: new_rest_gen },
          result }
      _ ->
        many
