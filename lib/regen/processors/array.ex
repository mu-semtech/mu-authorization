...
defmodule Array do
  ...
  defp ensure_child_generator_exists( %Array{ child_generator: :none, elements: [element|_], state: state } = arr ) do
    %{ arr |
       child_generator: dispatch_generation( element, state ) }
  end
  ...
  defp ensure_child_result_exists( %Array{ last_child_result: :none, child_generator: gen } = arr ) do
    case emit( gen ) do
      { :ok, new_gen, new_state } ->
        { :ok,  %{ arr |
                   last_child_result: new_state,
                   child_generator: new_gen } }
      _ -> { :fail }
    end
  end
  ...
  defp ensure_rest_generator_exists( %Array{ rest_generator: :none,
                                             elements: [_|rest],
